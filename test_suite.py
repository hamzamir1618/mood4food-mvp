...
test_cases = [
    {
        "query": "food under 300 rupees, no dairy",
        "expected_pruned": ["dairy"],
        "budget_priority": 0.5,
    },
    {
        "query": "I am vegetarian, no meat no chicken",
        "expected_pruned": ["meat"],
        "budget_priority": 0.2,
    },
    {"query": "cheap food without nuts", "expected_pruned": ["nuts"], "budget_priority": 1.0},
    {
        "query": "high protein food, I am allergic to shellfish",
        "expected_pruned": ["shellfish"],
        "budget_priority": 0.1,
    },
    {
        "query": "something spicy for dinner, no gluten",
        "expected_pruned": ["gluten"],
        "budget_priority": 0.5,
    },
    ...
    {
        "query": "gym food with high protein under 800",
        "expected_pruned": [],
        "budget_priority": 0.4,
    },
    ...
    {
        "query": "shrimp and fish are bad for me",
        "expected_pruned": ["shellfish", "fish"],
        "budget_priority": 0.5,
    },
    {"query": "sweet and creamy dessert under 500", "expected_pruned": [], "budget_priority": 0.6},
    {
        "query": "no wheat gluten free please 400",
        "expected_pruned": ["gluten"],
        "budget_priority": 0.7,
    },
...
def run_tests():
    ...
    for idx, case in enumerate(test_cases, 1):
        print(
            f"Running Test {idx}: Query: '{case['query']}' | Budget Weight: {case['budget_priority']}"
        )
        try:
            # Step 1: Submit Query
            ...
            if response.status_code != 200:
                raise AssertionError(
                    f"Expected status 200 on /submit, got {response.status_code}. Detail: {response.text}"
                )
            ...
            for expected in case["expected_pruned"]:
                if expected not in allergens_pruned:
                    raise AssertionError(
                        f"Expected allergen '{expected}' to be pruned, but got: {allergens_pruned}"
                    )
            ...
            if recalc_resp.status_code != 200:
                raise AssertionError(
                    f"Expected status 200 on /recalculate, got {recalc_resp.status_code}. Detail: {recalc_resp.text}"
                )
            ...
            if not all_candidates:
                ...
                logger.warning(
                    f"No candidates available for query: '{case['query']}'. Validation skipping winner check."
                )
            else:
                if case["budget_priority"] == 1.0:
                    lowest_price = min(
                        cand.get("price_pkr", float("inf")) for cand in all_candidates
                    )
                    winner_price = winner.get("price_pkr")
                    if winner_price != lowest_price:
                        raise AssertionError(
                            f"Budget weight is 1.0 but winning dish ({winner.get('name')}) price {winner_price} is not the lowest price ({lowest_price})."
                        )

            print(
                f"PASS Test {idx}: Allergens correctly pruned and recalculated successfully. Winner: {winner.get('name')} at {winner.get('price_pkr')} PKR."
            )
            passed_tests += 1
