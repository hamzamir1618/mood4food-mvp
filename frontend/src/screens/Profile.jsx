...
export default function Profile({ user, onClose, onSignedOut, toast }) {
  ...
  const remove = () =>
    save(() => api.deleteAccount(password), null).then(() => {
      onSignedOut();
        ...
        <div className="profile-col">
          {learned && (
            <div style={{ background: 'var(--ink)', color: 'var(--paper)', padding: 18, marginTop: 18 }}>
              <div className="lab lab-sm" style={{ color: 'var(--accent)' }}>
                What I've learned
              </div>
            ...
            {DIMS.map(([key, label]) => (
              <div key={key} style={{ borderTop: '1px solid var(--rule)', padding: '10px 0' }}>
                <div className="between">
                  ...
                  <span className="bod-b" style={{ fontSize: 20 }}>
                    {pct(taste[key])}
                  </span>
                </div>
                <input
        ...
        <div className="profile-col">
              ...
              <div className="pt-8">
                {similar.matches?.map((m, i) => (
                  <div key={i} className="between" style={{ padding: '6px 0' }}>
                    ...
                    <span className="bod-b" style={{ fontSize: 18 }}>
                      {pct(m.similarity)}
                    </span>
                  </div>
                ))}
                {similar.matches?.length > 0 && <div className="lab lab-sm muted pt-8">No one is identified.</div>}
          ...
          <div className="pt-8">
            <a
              className="between"
              href={api.exportUrl}
              style={{ borderTop: '1px solid var(--rule)', padding: '14px 0', textDecoration: 'none' }}
            >
            ...
            <button
              className="between"
              onClick={signOut}
              style={{ width: '100%', borderTop: '1px solid var(--rule)', padding: '14px 0' }}
            >
