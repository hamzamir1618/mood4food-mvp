...
export default function Done({ dish, onRestart }) {
  ...
  return (
    <div className={`screen done${dish?.image_url ? '' : ' no-photo'}`}>
      <div className="done-head">
        <div className="lab accent rise pt-16">Settled</div>
        <div className="rule-thick draw" style={{ marginTop: 8 }} />

        <h2 className="h2 done-name mask pt-16">{dish?.name}</h2>
      ...
      {dish?.image_url && (
        <div className="photo done-photo wipe" style={{ marginTop: 18 }}>
          <img src={dish.image_url} alt={dish.name} />
        </div>
      )}

      <div className="between done-price pt-16">
        <div className="bod-b" style={{ fontSize: 30 }}>
